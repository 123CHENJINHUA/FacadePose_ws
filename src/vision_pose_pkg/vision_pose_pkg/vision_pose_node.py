...
class VisionPoseEstimatorNode(Node):
    def __init__(self):
        ...
        # --- perf/log ---
        self.declare_parameter('stats_log_period_sec', 5.0)
        self.declare_parameter('waiting_log_throttle_sec', 2.0)
        ...
        self.waiting_log_throttle_sec = float(self.get_parameter('waiting_log_throttle_sec').value)
        ...
        # This message can spam if node restarts; keep it once
    ...
    def process_and_publish(self):
        ...
        if not self.camera_info_received:
            self.get_logger().info('No camera info yet, cannot process data', throttle_duration_sec=self.waiting_log_throttle_sec)
            return
        # 需要同步后的彩色图 + 点云
        if self.latest_color is None or self.latest_cloud is None or self.synced_stamp is None:
            # Throttle to avoid terminal IO stalls
            self.get_logger().info('Waiting for synchronized messages...', throttle_duration_sec=self.waiting_log_throttle_sec)
            return
        ...
        try:
            ...
            if current_time - self.last_log_time > self.stats_log_period_sec:
